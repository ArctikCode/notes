...
if(!$result){
    echo '<div class="alert alert-danger">Error accessing the database.</div>'; exit;
}
...
$time = time();
$status = 'pending';
$sql = "INSERT INTO forgotpassword (`user_id`, `forgot_password_key`, `time`, `status`) VALUES ('$user_id', '$key', '$time', '$status')";
$result = mysqli_query($link, $sql);
if(!$result){
...
$message .= "http://bike.pack/Notes/resetpassword.php?user_id=$user_id&key=$key";
if(mail($email, 'Reset your password', $message, 'From:'.'contact@notes.fr')){
