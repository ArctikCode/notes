...
//echo "<div style = 'margin-top:200px;'>cookie" . $_COOKIE['rememberme']  . "</div>";
//echo "<div style = 'margin-top:200px;'>session ID" . $_SESSION['user_id']  . "</div>";
//echo "<div style = 'margin-top:200px;'>session ID" . $_SESSION['user_id']  . "</div>";


//If the user is not logged in & rememberme cookie exists
if(isset($_SESSION['user_id']) && !empty($_COOKIE['rememberme'])){
    //ALTERNATIVE WAY OF CHECKING : array_key_exists('user_id', $_SESSION)
    //f1: COOKIE: $a . "," . bin2hex($b)
    //f2: hash('sha256', $a)
    echo "<div style = 'margin-top:200px;'>" . $_COOKIE['rememberme']  . "</div>";
    ...
    if(!$result){
        echo  '<div class="alert alert-danger">Sorry, Looks like we don\'t remember you.</div>'; 
        exit;
    }
    $count = mysqli_num_rows($result);
    if($count !== 1){
        echo '<div class="alert alert-danger">We failed to access your data. Please Log in.</div>';
        exit;
    }
   ...
   if(!hash_equals($row['f2authentificator2'], $f2authentificator2)){
        echo '<div class="alert alert-danger">Identification failed.</div>';
    }else{
        ...
        /*we use INSERT and not UPDATE as the USER might have more than one device.*/
        ...
        header("location:mainPageLoggedIn.php");//redirect to the mainpage
